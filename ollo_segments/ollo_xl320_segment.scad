...
//
module ollo_xl320_side_segment(nLayer=1, width=OlloSegmentWidth) {
  add_three_ollo_segment(nLayer, width)
  add_ollo_straight_segment(2*OlloSpacing, nLayer, width)
  three_ollo_stop_segment(nLayer, width);
}

module add_ollo_xl320_side_segment(nLayer=1, width=OlloSegmentWidth) {
  ollo_xl320_side_segment(nLayer, width);
  translate([0, 4*OlloSpacing, 0])
  for(i = [0 : $children - 1])
  children(i);
}

module ollo_xl320_side_start_segment(nLayer=1, width=OlloSegmentWidth) {
  translate([0, -OlloSpacing/2, 0])
    ollo_xl320_side_segment(nLayer, width);
}

module add_ollo_xl320_side_start_segment(nLayer=1, width=OlloSegmentWidth) {
...
module ollo_xl320_side_stop_segment(nLayer=1, width=OlloSegmentWidth) {
  ollo_xl320_side_segment(nLayer, width);
}

module add_ollo_xl320_side_stop_segment(nLayer=1, width=OlloSegmentWidth) {
...
nLayer = 1;
if (p==1) {
  add_ollo_xl320_side_start_segment()
